...
class Reader{
  ...
  public Flight[] readIn(int start, int end, String filter) {
    ...
    for(int i = start; i < end; i++) {
      ...
      if(useThis) {
        ...
        flights.add(new Flight(parts[0], parts[1], parts[2], parts[3], parts[4].replace("\"", ""), parts[6], parts[7], parts[8], parts[9].replace("\"", ""), parts[11], parts[12], parts[13], parts[14], parts[15], parts[16], diverted, cancelled, distance));
      } //<>//
      useThis = true;
    }
    
    Flight[] returnFlights = new Flight[flights.size() - 1];
    
    for(int i = 0; i < returnFlights.length; i++) { //<>//
      returnFlights[i] = flights.get(i);
      System.out.printf("%s%n", returnFlights[i].toString());
